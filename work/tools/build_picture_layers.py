...
OUT = os.path.join(ROOT, 'engine', 'data', 'picture-layers.json')
...
for rid, path in RES.items():
    ...
    parts = base.split('_')
    # Register EVERY prefix alias, not just parts[:-1].  With only `parts[:-1]`,
    # `mou_back01` is filed under `mou`, so a table name like `rnd_mou_back` ->
    # key `mouback` finds nothing and the whole layer silently disappears
    # (that is how pictures 103/104 lost their 远山 band).
    for i in range(1, len(parts)):
        fam['_'.join(parts[:i]).lower().replace('_', '')].append(int(rid))
    # Also answer the TABLE's spelling when only the trailing number differs:
    # `mou_back01` on disk vs `rnd_mou_back` in the table -> key `mouback`
    # (stripping only parts[:-1] gives `mou`, which never matches).
    fam[re.sub(r'\d+$', '', base).lower().replace('_', '')].append(int(rid))
...
# --------------------------------------------------------------------- art size
# The pose rule below needs the sprite's real pixel size.  Read the PNG header
# directly: PIL is not a dependency of this build.
IMGDIR = os.path.join(ROOT, 'web', 'resource', 'China', 'images')
_size_cache = {}


def art_size(rid):
    """(w, h) of the shipped PNG for a resId, or None."""
    if rid in _size_cache:
        return _size_cache[rid]
    rel = RES.get(str(rid))
    path = os.path.join(IMGDIR, rel + '.png') if rel else None
    size = None
    ...
    _size_cache[rid] = size
    return size


# ---------------------------------------------------------------- POSE PLACEMENT
# Recovered from the game's OWN photograph-gif spines, not guessed:
# `animpictureData.pic_map {100:1, 104:2, 2000:3}` names `gif_1/2/3` under
# resource/China/animation/gif_photo, whose slot names are the very same art the
# Picture table references.  The role skeleton's ROOT BONE is the sprite's
# bottom-centre, and its screen position is
#
#     (250 + frogPos.x,  350 + frogPos.y)          # 250 = canvas centre, 350 = bottom
#
#   pic 100  predicted (349, 270)   spine root (349.63, 274.29)   Δ (0.6, 4.3)
#   pic 2000 predicted (164, 275)   spine root (162.67, 269.93)   Δ (1.3, 5.1)
#   pic 104  y predicted 270        spine root 276.27            Δ 6.3
#            (its gif is a WALK cycle on a scrolling skeleton, so x is not comparable)
#
# The old rule read POSE_OY=175 as the canvas MIDDLE and used the result as the
# sprite's TOP-LEFT, which lifted every frog ~175-h px: `frogPos.y` spans only
# -75..-94, so the frog landed at y 81..95 with nothing under it -- "小青蛙飞在天上".
# A fourth, independent check: pose art that is exactly 500x350 lands at (0,0) under
# this rule, and 445x238 art with frogPos (-28,1) lands at (-0.5, 113), i.e. flush
# with the frame instead of 54% cropped.
POSE_ANCHOR_X = int(os.environ.get('POSE_ANCHOR_X', '250'))
POSE_ANCHOR_Y = int(os.environ.get('POSE_ANCHOR_Y', '350'))
POSE_OX = POSE_ANCHOR_X                       # kept: older env vars still work
POSE_OY = POSE_ANCHOR_Y
POSE_SCALE = float(os.environ.get('POSE_SCALE', '1'))


def pose_xy(rid, pos):
    """Top-left corner for a pose sprite: bottom-centre anchor minus (w/2, h)."""
    size = art_size(rid)
    w, h = size if size else (0, 0)
    ax = POSE_ANCHOR_X + (pos.get('x') or 0)
    ay = POSE_ANCHOR_Y + (pos.get('y') or 0)
    # 17 of the 353 rows carry a POSITIVE frogPos.y (and 72 traveller slots do too),
    # which a bottom reference would push clean off the canvas -- pics 1012-1015
    # (y=111) would lose their frog entirely. Those rows use a reference frame we
    # could NOT recover, so rather than invent one we keep the sprite VISIBLE: the
    # anchor is clamped into the frame. Every other row is untouched by this.
    ax = min(max(ax, 0), 500)
    ay = min(ay, 350)
    return (int(round(ax - w / 2.0)), int(round(ay - h)))


# ------------------------------------------------------------------ SCENERY y
# `view` is {x:0, y:0} in ALL 351 rows, so it carries NO placement -- the old code
# emitted every scenery layer at (0,0), which dropped the ground/roof/sea bands at
# the TOP of the frame (and left the bottom half empty sky).  What the official
# spines actually show:
#   * a full-canvas backdrop really is (0,0)                     (3/3 checked)
...
#   * art TALLER than the frame (mumianhua_back 500x1000, panda 500x933, rain
#     1000x500 -- 82 of 480 scenery layers) is anchored the same way; leaving it at
#     (0,0) showed only its top 35%, which is sky.
# Bands that sit mid-frame (`shan` at y 218.88) are NOT covered by a rule we could
# confirm, so they fall through to (0,0) and were checked by eye in the contact
# sheet (work/probe/render_layers_probe.py).
# The names below are the GROUND / FOREGROUND bands.  `sea`/`flower` are
# deliberately NOT in this list even though they sit low in a scene: the official
# gif data puts `sea01` at y 98.75 and `flower01` at y 106.36, i.e. mid-frame, so
# bottom-aligning them would be wrong in the other direction.
BOTTOM_BAND = re.compile(
    r'^(earth|soil|ground|dimian|rail|road|roof|zg_?\d*|lu\d*|wood|stone|shizi'
    r'|shidun|ground|pipa_fore|tulou|wheat|field)', re.I)


def scenery_xy(rid, name, vx, vy):
    """Only the rule the official data actually proves; everything else keeps the
    table's own (0,0) instead of us inventing a placement."""
    if BOTTOM_BAND.match(name or ''):
        size = art_size(rid)
        if size:
            return int(vx), int(350 - size[1])       # bottom edge flush with the frame
...
for row in PICS:
    ...
    for pn in pose_names:
        ...
        if rid:
            px, py = pose_xy(rid, pos)
            layers.append({'layer': [int(rid), px, py], 'scale': POSE_SCALE})
        else:
    ...
    for i, tn in enumerate(row.get('travelerPose') or []):
        ...
        if rid:
            tx, ty = pose_xy(rid, p)
            travellers.append({'layer': [int(rid), tx, ty], 'scale': POSE_SCALE})
        else:
            # Not a pose after all: fall back to scenery placement (the old code
            # offset it by the pose origin, which is meaningless for scenery).
            if not emit(tn, vx, vy, ptype):
