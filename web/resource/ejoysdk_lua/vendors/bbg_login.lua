...
local CHANNEL = "BBG_LOGIN"
local UNISDK_CHANNEL = "BBG_LOGIN"
local EM = require "ejoysdk_lua.ejoysdk_module"
local TAG = EM.MODULE.VENDORS.BBG_LOGIN
...
local should_notify_cp = false

local BBG_HAS_LOGINED = E.LazyKeyStore:New('BBG_HAS_LOGINED', false, false, false)
...
function M.init(opt, cb)
    ...
    UNI.register_logout_listener(UNISDK_CHANNEL, function(ext_params)
        BBG_HAS_LOGINED:set(nil)
        if should_notify_cp then
    ...
    -- 绑定后bbg的登出没机会主动调用，需要依赖通知
    ET.subscribe(ET.gangplank.LOGOUT, M.logout)
...
function M.logout(manual)
    should_notify_cp = (not manual)
    if BBG_HAS_LOGINED:get() then
