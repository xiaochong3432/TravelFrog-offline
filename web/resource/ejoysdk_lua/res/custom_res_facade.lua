...
function M.get_res_state(_res_key)
    return ERF.get_res_state(ECC.NAMESPACE.QZ_CUSTOM_RES, _res_key)
end
...
function M.remove_res_version(_res_key, res_version)
    ER.remove_res_version(M.NAMESPACES.QZ_CUSTOM_RES, _res_key, res_version)
end
...
function M.repair(res_key)
    ER.publish_using_res_version(M.NAMESPACES.QZ_CUSTOM_RES, res_key, nil, nil, nil)
end
...
--[[
    删除配置文件，for lua的使用场景，lua正在使用中，只能删除配置文件，不删除资源
]]
