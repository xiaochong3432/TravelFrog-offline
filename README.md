...
当前开发安装版 **3.1**（`versionCode=5`）使用 `com.travelfrog.offline`。实际提供的网盘 V3 也是此包名，但其签名与本地开发包不同，**当前开发包不能直接覆盖该 V3**；正式升级须使用 V3 原签名密钥。详见 [实包身份核验及升级规范](docs/构建与打包.md#升级与存档规范31)。
...
用户提供的网盘 V3 实包使用 `com.travelfrog.offline`，但与开发包签名不同；本地另有 `com.frog.offline` 历史包，不能据此推断全部网盘版本的身份。
正式发布前必须拿实际分发的旧 APK 核对包名、签名和存储地址；不能改包名或卸载旧版解决升级冲突。
