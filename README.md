...
当前开发安装版 **3.1**（`versionCode=5`）使用 `com.travelfrog.offline`。网盘 V1–V3 发布路线使用 `com.frog.offline`，两者不能互相覆盖升级；**不得把开发包作为旧发布版的升级包**。升级须保持目标旧版的包名、签名和 WebView 存储地址。详见 [升级与存档规范](docs/构建与打包.md#升级与存档规范31)。
...
网盘 V1–V3 的升级目标是 `com.frog.offline`；`android-apk/` 的开发安装版是 `com.travelfrog.offline`。
两者存档隔离。正式发布前必须拿实际分发的旧 APK 核对签名和存储地址，不能只改开发包的 `applicationId`，也不能卸载旧版解决冲突。
